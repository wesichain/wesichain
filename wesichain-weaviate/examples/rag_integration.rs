...
pub async fn run_core_flow(
    ...
    let inserted_ids = docs.iter().map(|doc| doc.id.clone()).collect::<Vec<_>>();

    store.add(docs).await?;

    let results = store.search(&[0.98, 0.02, 0.0], 2, None).await?;

    store.delete(&inserted_ids).await?;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let class_name =
        std::env::var("WEAVIATE_CLASS").unwrap_or_else(|_| "WesichainDocs".to_string());

    let builder = WeaviateVectorStore::builder()
        .base_url(base_url)
        .class_name(class_name)
        .auto_create_class(true);
    let builder = match std::env::var("WEAVIATE_API_KEY") {
