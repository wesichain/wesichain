...
use wesichain_graph::{
    ExecutionOptions, GraphBuilder, GraphError, GraphState,
    InMemoryCheckpointer, StateSchema, StateUpdate, Checkpointer
};
...
impl Runnable<GraphState<DemoState>, StateUpdate<DemoState>> for RecordNode {
    async fn invoke(&self, _: GraphState<DemoState>) -> Result<StateUpdate<DemoState>, WesichainError> {
...
async fn test_interrupt_after_resume() {
    ...
    let graph = GraphBuilder::<DemoState>::new()
        .add_node("A", RecordNode { name: "A".to_string(), delay: None })
        .add_node("B", RecordNode { name: "B".to_string(), delay: None })
        .add_node("C", RecordNode { name: "C".to_string(), delay: None })
        .add_edge("A", "B")
        .add_edge("B", "C")
        .set_entry("A")
    ...
    let result = graph.invoke_graph_with_options(GraphState::new(DemoState::default()), options).await;
    
    // Should fail with Interrupted
    match result {
        Err(GraphError::Interrupted) => assert!(true),
        _ => panic!("Expected Interrupted error, got {:?}", result),
    }

    // Load Checkpoint
    let checkpoint = checkpointer.load("thread-1").await.unwrap().expect("Checkpoint not found");
    assert_eq!(checkpoint.node, "A");
    // Queue should have "B" (successor of A)
    assert!(checkpoint.queue.iter().any(|(n, _)| n == "B"));

    // Resume
    let resumed = graph.resume(checkpoint, ExecutionOptions::default()).await.unwrap();
...
async fn test_parallel_interrupt_resume() {
    ...
    let graph = GraphBuilder::<DemoState>::new()
        .add_node("A", RecordNode { name: "A".to_string(), delay: None })
        .add_node("B", RecordNode { name: "B".to_string(), delay: Some(Duration::from_millis(10)) })
        .add_node("C", RecordNode { name: "C".to_string(), delay: Some(Duration::from_millis(100)) })
        .add_edges("A", &["B", "C"])
        .set_entry("A")
    ...
    assert!(queue_names.contains(&"C".to_string()), "Queue should contain aborted node C");
